...
<div>
  <!-- 专业工作-签证 -->
  <el-dialog :visible.sync="visaDialog" size="small" custom-class="icms-dialog">
    <span slot="title">专业工作-签证</span>
    <header v-if="!this.$store.state.app.wholeSingleRouter">
      <el-form label-position="right" label-width="35%">
        <el-row :gutter="16">
          <el-col :sm="6">
            <el-form-item label="签证编号：" class="w">
              <el-input v-model="visaSearch.jobNumber" class="w" size="small" placeholder="请输入编号"></el-input>
            </el-form-item>
          </el-col>
          <el-col :sm="6">
            <el-form-item label="签证名称：" class="w">
              <el-input v-model="visaSearch.jobName" class="w" size="small" placeholder="请输入文件名称"></el-input>
            </el-form-item>
          </el-col>
          <el-col :sm="6">
            <el-form-item label="对应合同：" class="w">
              <el-select class="w" v-model="visaSearch.xxContract" placeholder="请选择对应合同" clearable>
          ...
          <el-col :sm="6">
            <el-form-item label="流程状态：" class="w">
              <el-select class="w" v-model="visaSearch.processState" placeholder="请选择流程状态" clearable>
        ...
        <div class="tc wait-header-btn f13 mb-20">
          <el-button @click="visaSearchBtn" class="btn-orange shadow mt-15" type="primary"><i class="iconfont icon-shiliangzhinengduixiang f14 mr-5"></i>查 询</el-button>
    ...
    <el-table class="noShadow mt-10" :data="visaData" style="width: 100%" :loading="visaSearch.loading" highlight-current-row>
      ...
      <el-table-column min-width="150" label="签证内容">
        <template slot-scope="scope">
      ...
      <el-table-column prop="subtractAmount" width="150" label="核（增）减金额（元）">
        <template slot-scope="scope">
          {{scope.row.subtractAmount?scope.row.subtractAmount:"--"}}
        </template>
      </el-table-column>
      <el-table-column prop="createTime" width="180" label="下发时间"></el-table-column>
      <el-table-column prop="xxDateStr" width="120" label="签证日期"></el-table-column>
    ...
    <footer class="mt-50 cb pagination" v-if="visaSearch.total&&!this.$route.query.projectId">
      <el-pagination @current-change="visaPageChange" :current-page.sync="visaSearch.pageNum" layout="prev, pager, next, jumper" :total="visaSearch.total" class="fr">
      </el-pagination>
      <span class="fr f13 color-8 pagination-text mr-15 mt-8">每页10条，共{{visaSearch.total}}条记录</span>
    </footer>
  </el-dialog>
  <!-- 新增 - 签证基本信息  -->
  <el-dialog :visible.sync="creatVisaDialog" size="small" custom-class="icms-dialog">
    <span slot="title">{{creatVisaData.title}}</span>
    <div class="dialog-box">
      <el-form :model="creatVisaData" ref="creatVisaData" :inline="true" label-width="35%" :rules="creatVisaDataRules">
        <div class="icms-dialog-content pr p15 mt-10" v-if="creatVisaData.editSave">
          <nav class="pa">
            专业工作要求
          </nav>
          ...
          <el-row :gutter="16" class="mt-15">
            ...
            <el-col :sm="8" :xs="12">
              <div class="grid-content">{{visaSingleData.jobRequirement?visaSingleData.jobRequirement:"--"}}</div>
            </el-col>
          </el-row>
          ...
          <el-row :gutter="16" class="mt-15">
            ...
            <el-col :sm="8" :xs="12">
              <div class="grid-content">{{visaSingleData.jobCommitDeadlineDateStr?visaSingleData.jobCommitDeadlineDateStr:"--"}}</div>
            </el-col>
            <el-col :sm="10">
              <el-col :sm="12" :xs="12">
              ...
              <el-col :sm="12" :xs="12">
                <div class="grid-content">{{visaSingleData.createTimeStr?visaSingleData.createTimeStr:"--"}}</div>
              </el-col>
            </el-col>
          </el-row>
        </div>
        <div class="icms-dialog-content pr" :class="{'mt-40': creatVisaData.editSave}" v-if="creatVisaData.editSave">
          <nav class="pa">
            专业工作要求
          </nav>
          ...
          <el-row :gutter="16">
            <el-col :sm="14">
              <el-form-item label="签证编号：" class="mt-10 w creat-new-form" prop="jobNumber">
                <el-input class="w" v-model="creatVisaData.jobNumber" size="small" placeholder="请填写签证编号"></el-input>
              </el-form-item>
            </el-col>
            <el-col :sm="10">
              <el-form-item label="签证名称：" class="mt-10 w creat-new-form" prop="jobName">
                <el-input class="w" v-model="creatVisaData.jobName" size="small" placeholder="请填写签证名称"></el-input>
              </el-form-item>
            </el-col>
          </el-row>
          ...
          <el-row :gutter="16">
            <el-col :sm="14">
              <el-form-item label="签证内容：" class="mt-10 w creat-new-form" prop="xxContent">
                <el-input type="textarea" class="w" v-model="creatVisaData.xxContent" size="small" placeholder="请填写签证内容"></el-input>
              </el-form-item>
            </el-col>
          </el-row>
          <el-row :gutter="16">
            <el-col :sm="14">
              <el-form-item label="对应合同：" class="mt-10 w creat-new-form" prop="xxContract">
                <el-select v-model="creatVisaData.xxContract" placeholder="请选择对应合同" clearable>
                  <el-option v-for="item in contractList" :key="item.jobName" :label="item.jobName" :value="item.jobName">
                  </el-option>
                </el-select>
              </el-form-item>
          ...
          <el-row :gutter="16">
            <el-col :sm="14">
              <el-form-item label="审核金额（元）：" class="mt-10 w creat-new-form" prop="validationAmount">
                <el-input type="number" class="w" v-model="creatVisaData.validationAmount" size="small" placeholder="请填写审核金额"></el-input>
              </el-form-item>
            </el-col>
            <el-col :sm="10">
              <el-form-item label="送审金额（元）：" class="mt-10 w creat-new-form" prop="reviewAmount">
                <el-input type="number" class="w" v-model="creatVisaData.reviewAmount" size="small" placeholder="请填写送审金额"></el-input>
              </el-form-item>
            </el-col>
          </el-row>
          <el-row :gutter="16">
            <el-col :sm="14">
              <el-form-item label="核（增）减金额（元）：" class="mt-10 w creat-new-form" prop="subtractAmount">
                <el-input type="number" class="w" v-model="creatVisaData.subtractAmount" size="small" placeholder="请填写核（增）减金额"></el-input>
              </el-form-item>
            </el-col>
            <el-col :sm="10">
              <el-form-item label="签证时间：" class="mt-10 w creat-new-form" prop="xxDateStr">
                <el-date-picker :editable="false" v-model="creatVisaData.xxDateStr" class="w" size="small" type="date" placeholder="请选择发起时间"></el-date-picker>
              </el-form-item>
          ...
          <el-row :gutter="16">
            <el-col :sm="14">
              <el-form-item label="备注：" class="mt-10 w creat-new-form" prop="remark">
                <el-input type="textarea" class="w" v-model="creatVisaData.remark" size="small" placeholder="请填写备注"></el-input>
              </el-form-item>
            </el-col>
          </el-row>
        </div>
        <div class="icms-dialog-content pr" :class="{'mt-40': creatVisaData.editSave}" v-if="creatVisaData.newSave">
          <nav class="pa">
            专业工作要求
          </nav>
          <el-row :gutter="16">
          ...
          <el-row :gutter="16">
            <el-col :sm="14">
              <el-form-item label="工作要求：" class="mt-10 w creat-new-form" prop="jobRequirement">
                <el-input type="textarea" class="w" v-model="creatVisaData.jobRequirement" size="small" placeholder="请填写工作要求"></el-input>
              </el-form-item>
            </el-col>
          </el-row>
          <el-row :gutter="16">
            <el-col :sm="14">
              <el-form-item label="工作上报截止日期：" class="mt-10 w creat-new-form" prop="jobCommitDeadlineDateStr">
                <el-date-picker :editable="false" v-model="creatVisaData.jobCommitDeadlineDateStr" class="w" size="small" type="date" placeholder="请选择工作上报截止日期"></el-date-picker>
              </el-form-item>
            </el-col>
          </el-row>
        </div>
        <div class="icms-dialog-content  pr mt-40" v-if="creatVisaData.fileList">
          <nav class="pa">
            专业材料附件
          </nav>
          <el-table class="noShadow" :data="creatVisaData.enclosureFiles" style="width: 100%">
            ...
            <el-table-column label="操作" width="120">
              <template slot-scope="scope">
                  <el-button v-if="creatVisaData.delShow" @click="delWholeAnalysisFile(scope.row,scope.$index)" class="color-green" type="text" size="small">删除</el-button>
                  </template>
            </el-table-column>
          </el-table>
        </div>
        ...
        <div class="dialog-footer tc mt-40">
          <el-button :loading="buttonLoadingSubmit" v-if="creatVisaData.editSave&&creatVisaData.editShowBtn" class="btn-green shadow mt-15" type="primary" @click='submitSaveVisaData("creatVisaData")'><i class="iconfont icon-save f14 mr-5"></i>提 交</el-button>
          <el-button :loading="buttonLoading" v-if="creatVisaData.editSave" class="btn-orange shadow mt-15" type="primary" @click='editSaveVisaData("creatVisaData")'><i class="iconfont icon-save f14 mr-5"></i>保 存</el-button>
          <el-button :loading="buttonLoading" v-if="creatVisaData.newSave" class="btn-orange shadow mt-15" type="primary" @click='saveVisaData("creatVisaData")'><i class="iconfont icon-save f14 mr-5"></i>保 存</el-button>
          <el-button class="btn-white shadow mt-15" @click='creatVisaDialog=false'><i class="iconfont icon-quxiao1 f13 mr-5"></i>取 消</el-button>
        </div>
      </el-form>
    </div>
  </el-dialog>
  ...
  <el-dialog :visible.sync="visaSingleDialog" size="small" custom-class="icms-dialog">
    <span slot="title">查看 - 签证基本信息</span>
    <div class="dialog-box">
      ...
      <div class="icms-dialog-content pr p15 mt-40">
        ...
        <el-row :gutter="16" class="mt-15">
          <el-col :sm="6" :xs="12">
            <div class="grid-content tr">签证编号：</div>
          </el-col>
          <el-col :sm="8" :xs="12">
            <div class="grid-content">{{visaSingleData.jobNumber?visaSingleData.jobNumber:"--"}}</div>
          </el-col>
          <el-col :sm="10">
            <el-col :sm="12" :xs="12">
              <div class="grid-content tr">签证名称：</div>
            </el-col>
            <el-col :sm="12" :xs="12">
              <div class="grid-content">{{visaSingleData.jobName?visaSingleData.jobName:"--"}}</div>
            </el-col>
          </el-col>
        </el-row>
        ...
        <el-row :gutter="16" class="mt-15">
          <el-col :sm="6" :xs="12">
            <div class="grid-content tr">签证内容：</div>
          </el-col>
          <el-col :sm="8" :xs="12">
            <div class="grid-content">{{visaSingleData.xxContent?visaSingleData.xxContent:"--"}}</div>
          </el-col>
        </el-row>
        <el-row :gutter="16" class="mt-15">
          <el-col :sm="6" :xs="12">
          ...
          <el-col :sm="8" :xs="12">
            <div class="grid-content">{{visaSingleData.xxContract?visaSingleData.xxContract:"--"}}</div>
          </el-col>
        </el-row>
        ...
        <el-row :gutter="16" class="mt-15">
          ...
          <el-col :sm="8" :xs="12">
            <div class="grid-content">{{visaSingleData.validationAmount?visaSingleData.validationAmount:"--"}}</div>
          </el-col>
          <el-col :sm="10">
            <el-col :sm="12" :xs="12">
            ...
            <el-col :sm="12" :xs="12">
              <div class="grid-content">{{visaSingleData.reviewAmount?visaSingleData.reviewAmount:"--"}}</div>
            </el-col>
          </el-col>
        </el-row>
        <el-row :gutter="16" class="mt-15">
          ...
          <el-col :sm="8" :xs="12">
            <div class="grid-content">{{visaSingleData.subtractAmount?visaSingleData.subtractAmount:"--"}}</div>
          </el-col>
          <el-col :sm="10">
            <el-col :sm="12" :xs="12">
              <div class="grid-content tr">签证时间：</div>
            </el-col>
            <el-col :sm="12" :xs="12">
              <div class="grid-content">{{visaSingleData.xxDateStr?visaSingleData.xxDateStr:"--"}}</div>
            </el-col>
          </el-col>
        </el-row>
        <el-row :gutter="16" class="mt-15">
          ...
          <el-col :sm="8" :xs="12">
            <div class="grid-content">{{visaSingleData.remark?visaSingleData.remark:"--"}}</div>
          </el-col>
        </el-row>
      </div>
      <div class="icms-dialog-content pr mt-40">
        ...
        <el-table class="noShadow" :data="visaSingleData.files" style="width: 100%" :loading="visaSingleData.loading" highlight-current-row>
          <el-table-column type="index" label="编号" width="50" align="center"></el-table-column>
          <el-table-column min-width="170" prop="fileName" label="文件名称"></el-table-column>
    ...
    <div class="dialog-box">
      ...
      <div class="icms-dialog-content pr p15 mt-40">
        ...
        <el-row :gutter="16" class="mt-15">
          ...
          <el-col :sm="8" :xs="12">
            <div class="grid-content">{{visaSingleData.professionalDirector?visaSingleData.professionalDirector:"--"}}</div>
          </el-col>
          <el-col :sm="10">
            <el-col :sm="12" :xs="12">
      ...
      <div class="icms-dialog-content pr p15 mt-40">
        ...
        <el-row :gutter="16">
          <el-col :sm="24">
            <el-steps class="oa" :space="90" :active="visaSingleData.processStateNum" finish-status="success" :align-center="true" :center="true">
      ...
      <div class="icms-dialog-content pr mt-40">
        ...
        <el-table class="noShadow" :data="historyInfo" style="width: 100%" :loading="visaSingleData.loading" highlight-current-row>
          <el-table-column type="index" label="编号" width="50" align="center"></el-table-column>
          <el-table-column min-width="160" prop="processState" label="处理步骤"></el-table-column>
...
export default {
  ...
  watch: {
    creatVisaData: {
      handler: function(val, oldval) {
        this.creatVisaData.subtractAmount = (val.reviewAmount - val.validationAmount) ? (val.reviewAmount - val.validationAmount).toFixed(2):0;
      },
  ...
  methods: {
    exportWholeSingle() { //批量导出
      this.$confirm('确定要导出吗？生成文件时间可能较长，请耐心等待。', {
        ...
        let exportData = {
          ...
          jobNumber: this.visaSearch.jobNumber.trim(),
          xxContract: this.visaSearch.xxContract,
          jobName: this.visaSearch.jobName.trim(),
          processState: this.visaSearch.processState
        }
        this.$message({
        ...
        this.publicFun.httpRequest("get", `wholeprocess/exportConsultantJobs`, null, (status, data, message) => {
          if (status == 0) {
            let excelData = [
              ["序号", "所属专业", "专业咨询员", "工作类型", "签证编号", "签证名称", "签证内容", "对应合同", "送审金额（元）", "审核金额（元）", "核（增）减金额（元）", "下发时间", "签证日期", "当前状态", "备注"],
            ];
            for (let item in data) {
              let excelSaveArray = [item != "removeByValue" ? parseInt(item) + 1 : "", data[item].professional, data[item].consultant, data[item].jobType, data[item].jobNumber, data[item].jobName, data[item].xxContent, data[item].xxContract,
                data[item].reviewAmount, data[
                  item].validationAmount, data[item].subtractAmount, data[item].createTimeStr, data[item].xxDateStr, data[item].processState, data[item].remark
              ]
              excelData.push(excelSaveArray)
            };
            buildExcel(excelData, `《${this.projectInfo.projectName}》签证列表`);
    ...
    viewProgress(row) {
      ...
      this.visaSingleData = row;
      //console.log('项目信息', row);
      if (row.isDirectorAudit == "否") {
        this.stepData = ["专业工作", "项目经理初审", "追加附件", "项目经理终审", "完成"]
        switch (row.processState) {
          case "专业工作":
            this.visaSingleData.processStateNum = 0;
            break;
          case "项目经理初审":
            this.visaSingleData.processStateNum = 1;
            break;
          case "追加附件":
            this.visaSingleData.processStateNum = 2;
            break;
          case "项目经理终审":
            this.visaSingleData.processStateNum = 3;
            break;
          case "完成":
            this.visaSingleData.processStateNum = 5;
            break;
        }
        ...
        switch (row.processState) {
          ...
          case "分管副总审核":
            ...
            break;
          case "追加附件":
            this.visaSingleData.processStateNum = 4;
            break;
          case "项目经理终审":
            this.visaSingleData.processStateNum = 5;
            break;
          case "完成":
            this.visaSingleData.processStateNum = 7;
            break;
        }
    ...
    viewVisaSingle(row) { //查看签证信息
      this.visaSingleDialog = true;
      //console.log("签证信息", row);

      this.visaSingleData.loading = true;
      this.publicFun.httpRequest("get", `wholeprocess/getJobFilesByJobId?id=${row.id}`, null, (status, data, message) => {
        if (status == 0) {
          this.visaSingleData = row;
          this.visaSingleData.loading = false;
          this.visaSingleData.files = data;
        } else {
          this.$message.error(message);
        }
      });
    ...
    editVisaSingle(row,title) { //编辑技术核定信息
      this.visaSingleData = row;
      this.creatVisaDialog = true;
      this.creatVisaData.editShowBtn = title?false:true;
      this.creatVisaData.title = "修改 - 签证信息";
      this.creatVisaData.delShow = false;
      this.creatVisaData.editSave = true;
      this.creatVisaData.newSave = false;
      this.creatVisaData.fileList = true;

      this.creatVisaData.jobName = row.jobName;
      this.creatVisaData.xxContent = row.xxContent;
      this.creatVisaData.jobNumber = row.jobNumber;
      this.creatVisaData.xxContract = row.xxContract;
      this.creatVisaData.validationAmount = row.validationAmount;
      this.creatVisaData.reviewAmount = row.reviewAmount;
      this.creatVisaData.subtractAmount = row.subtractAmount;
      this.creatVisaData.xxDateStr = row.xxDateStr;
      this.creatVisaData.remark = row.remark;
      this.creatVisaData.editId = row.id;
      this.creatVisaData.editVersion = row.version;
      ...
      this.publicFun.httpRequest("get", `wholeprocess/getJobFilesByJobId?id=${row.id}`, null, (status, data, message) => {
        if (status == 0) {
          this.creatVisaData.enclosureFiles = data;
        } else {
          this.$message.error(message);
        }
      });
    ...
    confirmNextBtn() {
      let submitData = {
        id: this.creatVisaData.editId,
        version: this.creatVisaData.editVersion + 1,
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          let contractData = {
            ...
            "files_JDK": null,
            "files_QT": null,
            "files_QZ": this.contractData.uploadFiles.length > 0 ? JSON.stringify(this.contractData.uploadFiles) : null,
            "files_XJD": null,
            "files_ZJLXD": null,
            "htFiles": null,
            "id": this.creatVisaData.editId,
            ...
            "jobName": this.creatVisaData.jobName,
            "jobNumber": this.creatVisaData.jobNumber,
            "jobRequirement": null,
            "jobType": "签证",
            ...
            "receiveUnit": null,
            "remark": this.creatVisaData.remark,
            "reviewAmount": this.creatVisaData.reviewAmount,
            "sendPackagePerson": null,
            "sendTaskFiles": null,
            "sendUnit": null,
            "singleId": this.projectInfo.id,
            "startDateStr": null,
            "subtractAmount": this.creatVisaData.subtractAmount,
            "validationAmount": this.creatVisaData.validationAmount,
            "xxAmount": null,
            "xxContent": this.creatVisaData.xxContent,
            "xxContract": this.creatVisaData.xxContract,
            "xxDateStr": formatDateShort(this.creatVisaData.xxDateStr),
            "version": this.creatVisaData.editVersion
          }
          this.buttonLoading = true;
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          let contractData = {
            ...
            "isDynamicCost": null,
            "isSendMsg": null,
            "jobCommitDeadlineDateStr": formatDateShort(this.creatVisaData.jobCommitDeadlineDateStr),
            "jobName": this.creatVisaData.jobName,
            "jobNumber": this.creatVisaData.jobNumber,
            "jobRequirement": this.creatVisaData.jobRequirement,
            "jobType": "签证",
            "monthyApplyAmount": null,
            "monthyReplyAmount": null,
            "payType": null,
    ...
    creatVisaBtn() { //新建技术核定单
      ...
      this.creatVisaData.delShow = true;
      this.creatVisaData.editSave = false;
      this.creatVisaData.newSave = true;
      this.creatVisaData.fileList = false;
      if (this.contractData.uploadFiles.length > 0) {
        this.$refs.upload.clearFiles();
      }
      this.contractData.uploadFiles = [];
    ...
    visaContentChildBtn() { //签证子任务按钮
      this.visaSearch.jobNumber = '';
      this.visaSearch.jobName = '';
      this.visaDialog = true;
      this.getContractListArray(this.projectInfo.id);
      this.getVisaList();
    },
    visaSearchBtn() { //签证子任务查询
      this.getVisaList();
    },
    getVisaList() { //签证列表获取
      this.visaSearch.loading = true;
      let params = {
        page: this.visaSearch.pageNum,
        rows: 10,
        singleId: this.projectInfo.id,
        jobType: "签证",
        id: this.projectInfo.waitId ? this.projectInfo.waitId : null,
      ...
      this.publicFun.httpRequest("get", 'wholeprocess/listConsultantJobs', null, (status, data, message) => {
        if (status == 0) {
          ...
          this.visaSearch.total = data.total;
          this.visaSearch.loading = false;
        } else {
          this.$message.error(message);
        }
      }, params);
    ...
    uploadChildDataSuccess(response, file, fileList) { //签证文件上传
      let fileData = {
