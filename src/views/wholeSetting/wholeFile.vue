...
<div id="systemSetting-content" class="app-container">
  <header class="p20 bg-color-white shadow radius5">
    <div class="mt-5">
      <el-form :inline="true" ref="costFileData" :model="costFileData" class="demo-form-inline" label-width="100px">
        <el-form-item label="单位名称：" class="mt-10" prop="companyName">
          <el-input size="small" v-model="costFileData.companyName" placeholder="请输入单位名称"></el-input>
        </el-form-item>
        <el-form-item label="文件名称：" class="mt-10" prop="fileName">
          <el-input size="small" v-model="costFileData.fileName" placeholder="请输入文件名称"></el-input>
        </el-form-item>
        <el-form-item label="文件类型：" class="mt-10" prop="fileType">
          <el-select v-model="costFileData.fileType" size="small" placeholder="请选择文件类型" clearable>
    ...
    <div class="mt-30 mb-15 tc">
      ...
      <el-button v-if="roleMenus.creatFile" @click='addCostFile' type="primary" class="btn-blue shadow mt-15"><i class="iconfont icon-iconfont707 f13 mr-5"></i>新 增</el-button>
    </div>
  </header>
  <article class="mt-30">
    <el-table :data="costFileDataContent" v-loading="listLoading" fit highlight-current-row>
      ...
      <el-table-column align="center" label='操作' width="95">
        <template slot-scope="scope">
          <el-button type="text" class="color-green f14" @click="viewFileBtn(scope.row)">查看</el-button>
        </template>
      </el-table-column>
    </el-table>
  </article>
    ...
    <div class="dialog-box">
      <el-form :model="newCostFile" ref="newCostFile" class="demo-form-inline" label-width="120px" :rules="newCostFileRules">
        ...
        <div class="icms-dialog-content pr mt-40">
          ...
          <el-upload ref="upload" class="upload-file w" :on-remove="requireRemove" :on-error="fileError" :on-success="requireSuccess" drag action="icms/ftp/upload" accept="*" multiple>
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          let costFileDPost = {
            ...
            "moduleType": "wholeProcess",
            "wholeProcessFiles": this.costEditFilesArray.length != 0 ? JSON.stringify(this.costEditFilesArray) : null
          }
          this.buttonLoading = true;
